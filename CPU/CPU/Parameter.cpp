...
namespace NES
{
    ...
    {
        return static_cast<uint16_t>(ax + NES_Register::X);
    }

    uint16_t Parameter::ay(uint16_t ay)
    {
        return static_cast<uint16_t>(ay + NES_Register::Y);
    }
    ...
    // per http://wiki.nesdev.com/w/index.php/Addressing_modes ("Indexed
    ...
    // 16-bit increment involved). The C# original (CPU/CPU/Parameter.cs
    // zpx1) added zp+X and read `address+1` unwrapped - so e.g. zp=$FF,X=$00
    // read the pointer's high byte from real address $0100 (deep in the
    // stack page) instead of wrapping back to zero page's $00, producing a
    // completely wrong effective address whenever the pointer straddled the
    // $FF/$00 boundary.
    ...
    // FIXED (was a preserved C# bug, now corrected - found via nestest.nes):
    // per http://wiki.nesdev.com/w/index.php/Addressing_modes ("Zero Page
    ...
    // the zero page. The C# original (CPU/CPU/Parameter.cs zpx2) returned
    // the unwrapped 16-bit sum, so e.g. zp=$FF,X=$8A read real address
    // $0189 (deep in RAM) instead of wrapping back to $89.
    ...
    // FIXED (was a preserved C# bug, now corrected - found via nestest.nes,
    // same bug as zpx1 above, same fix): (zp),Y reads its 16-bit base
    ...
    uint16_t Parameter::zpy1(uint8_t zpy)
    {
        ...
        return static_cast<uint16_t>(temp + NES_Register::Y);
    }

    // FIXED (was a preserved C# bug, now corrected - found via nestest.nes,
    // same bug as zpx2 above, same fix): zp,Y also always stays within the
    // zero page.
    ...
    // FIXED (was a preserved C# bug, now corrected - found via nestest.nes;
    // unlike this session's other fixes, this one *adds* a hardware quirk
    // rather than removing a mistake): per
    ...
    // it, not "fix" it. The C# original (CPU/CPU/Parameter.cs
    // MemoryValueToAdress) always used the hardware-non-buggy `a + 1`. Fixed
    // to wrap within the page when `a`'s low byte is $FF, matching real
    // hardware.
